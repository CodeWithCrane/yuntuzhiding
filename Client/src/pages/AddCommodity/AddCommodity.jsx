...
const AddCommodity = () => {
    ...
    const handleSubmit = async () => {
        try {
            const res = await axios.post("/commodity", formData);
            if (res.status === 200) {
    ...
    return (
        <div style={{ clear: "both", padding: 24 }}>
                ...
                <Row>
                    <Col md={24} lg={12}>
                        <Form.Item name="商品名称" label="商品名称" rules={[{ required: true, message: "请输入商品名称" }]}>
                            <Input id="name" placeholder="请输入商品名称" onChange={handleChange}></Input>
                        </Form.Item>
                    </Col>
                    <Col md={24} lg={12}>
                        <Form.Item name="商品编号" label="商品编号" rules={[{ required: true, message: "请输入编号" }]}>
                            <Input id="commodityNumber" placeholder="请输入商品编号" onChange={handleChange}></Input>
                        </Form.Item>
                    </Col>
                </Row>
                <Row gutter={16}>
                    <Col md={24} lg={12}>
                        <Form.Item name="种类" label="种类" rules={[{ required: true }]}>
                            <Select placeholder="请选择种类" onChange={(value) => setFormData({ ...formData, category: value })}>
                ...
                <Row>
                    <Col md={24} lg={12}>
                        <Form.Item name="计量单位" label="计量单位" rules={[{ required: true }]}>
                            <Select placeholder="请选择单位" onSelect={(value) => setFormData({ ...formData, unit: value })}>
                    ...
                    <Col md={24} lg={12}>
                        <Form.Item name="价格" label="价格" rules={[{ required: true }]}>
                            <InputNumber id="price" defaultValue={0} onChange={(value) => setFormData({...formData, price: value})}></InputNumber>
                        </Form.Item>
                    </Col>
                </Row>
                <Row>
                    <Col md={24} lg={12}>
                        <Form.Item name="库存" label="库存" rules={[{ required: true }]}>
                            <InputNumber id="inventory" defaultValue={0} onChange={(value) => setFormData({...formData, inventory: value})}></InputNumber>
                        </Form.Item>
                    </Col>
                    <Col md={24} lg={12}>
                    </Col>
